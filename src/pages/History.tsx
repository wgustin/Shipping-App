...
import { Search, Plus, Copy, Check, ExternalLink, Printer, MoreHorizontal, Trash2 } from 'lucide-react';
...
export const History: React.FC<HistoryProps> = ({
  ...
  const handleClearHistory = async () => {
    ...
    if (window.confirm('Are you sure you want to delete all shipment history? This cannot be undone.')) {
      setIsClearing(true);
      try {
        await clearHistory();
      } finally {
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            ...
            <div className="flex items-center gap-4 w-full md:w-auto">
                ...
                {clearHistory && shipments.length > 0 && (
                  <Button 
                      variant="outline"
                      onClick={handleClearHistory} 
                      disabled={isClearing}
                      className="rounded-xl px-4 py-6 text-rose-600 border-rose-200 hover:bg-rose-50 hover:border-rose-300 font-bold transition-all flex items-center gap-2"
                  >
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="border-b border-slate-100 bg-slate-50/30">
                                <th className="px-4 py-5 text-[11px] font-bold text-slate-400 uppercase tracking-widest">DATE</th>
                                <th className="px-4 py-5 text-[11px] font-bold text-slate-400 uppercase tracking-widest">RECIPIENT</th>
                                <th className="px-4 py-5 text-[11px] font-bold text-slate-400 uppercase tracking-widest text-center">CARRIER</th>
                                <th className="px-4 py-5 text-[11px] font-bold text-slate-400 uppercase tracking-widest">TRACKING</th>
                                <th className="px-4 py-5 text-[11px] font-bold text-slate-400 uppercase tracking-widest">STATUS</th>
                                <th className="px-4 py-5 text-[11px] font-bold text-slate-400 uppercase tracking-widest">COST</th>
                                <th className="px-4 py-5 text-[11px] font-bold text-slate-400 uppercase tracking-widest text-center">ACTIONS</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-50">
                            {filteredShipments.map(s => (
                                ...
                                >
                                    ...
                                    <td className="px-4 py-5 whitespace-nowrap max-w-[180px]">
                                        ...
                                        <div className="flex items-center justify-center gap-2">
                                            {s.status !== 'cancelled' ? (
                                                <Button 
                                                    ...
                                                    className="rounded-lg px-3 py-2.5 border-none text-[11px] font-bold transition-all shadow-md shadow-slate-200"
                                                >
                                                ...
                                                <Button 
                                                    ...
                                                    className="rounded-lg px-3 text-slate-400 hover:text-slate-600 font-bold"
                                                >
                                                    View Details
                                                </Button>
                                            )}
